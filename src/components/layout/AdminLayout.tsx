...
import { LayoutDashboard, Users, List, Map, AlertTriangle, FileText, Settings, ShieldAlert, LogOut, Moon, Sun, Bell, Shield, ChevronLeft, ChevronRight, CheckSquare, Eye, MessageSquareWarning, BarChart2, Briefcase, FileSearch } from 'lucide-react';
...
export const AdminLayout = () => {
  ...
  const navCategories = [
    ...
    },
    {
      title: "Opérations",
      items: [
        { to: '/admin/affectations', icon: Briefcase, label: 'Affectations' },
        { to: '/admin/enquetes', icon: FileSearch, label: 'Enquêtes' },
        { to: '/admin/rumeurs', icon: MessageSquareWarning, label: 'Rumeurs' },
      ]
    },
    {
      title: "Analyse & Rapports",
      items: [
        { to: '/admin/statistiques', icon: BarChart2, label: 'Statistiques' },
    ...
    },
    {
      ...
      items: [
        ...
        { to: '/admin/audit', icon: ShieldAlert, label: 'Journal d\'Audit' },
        { to: '/admin/configuration', icon: Settings, label: 'Paramètres' },
      ]
    }
  ];
      ...
      <aside className={cn(
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!sidebarCollapsed && (
              <div>
                <h1 className="text-sm font-bold text-gray-900 dark:text-white leading-tight truncate">Admin SIGDU</h1>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-800">
          {!sidebarCollapsed ? (
            <div className="flex items-center gap-3 mb-4 px-2">
               <div className="w-9 h-9 rounded-full bg-emerald-100 dark:bg-emerald-900/50 flex items-center justify-center text-emerald-600 dark:text-emerald-500 font-bold shrink-0">
                 {user?.name?.charAt(0) || 'A'}
               </div>
               <div className="min-w-0 flex-1">
                 <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{user?.name}</p>
                 <p className="text-xs text-gray-500 truncate">Admin Système</p>
               </div>
            </div>
          ) : (
            <div className="flex justify-center mb-4">
              <div className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-900/50 flex items-center justify-center text-emerald-600 dark:text-emerald-500 font-bold" title={user?.name}>
                 {user?.name?.charAt(0) || 'A'}
              </div>
            </div>
          )}
          ...
          {allNavItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) => cn(
                ...
                isActive ? "text-emerald-600 dark:text-emerald-500" : "text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-300"
              )}
            >
              {({ isActive }) => (
                <>
                  <div className={cn("p-1 rounded-lg transition-colors", isActive && "bg-emerald-100 dark:bg-emerald-500/20")}>
